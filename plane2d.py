...
class Plane2d(AxesVisualizer):
    ...
    def get_rotated_plane(self, degree: float) -> Plane2d:
        ...
        rotated_x_axis = rotation_matrix @ self._get_local_coords_matrix()[:, 0]
        rotated_y_axis = rotation_matrix @ self._get_local_coords_matrix()[:, 1]
        
        return Plane2d(origin=self.origin, x_axis=rotated_x_axis[:2], y_axis=rotated_y_axis[:2])